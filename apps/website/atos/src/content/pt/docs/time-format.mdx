...
<Callout type=".">
  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '1rem', fontWeight: 'bold' }}>
    <FaClock size="1rem" />
    <span>Utilitário de formatação e controle de tempo:</span>
  </div>

O `TimeFormat` permite lidar com conversão de tempo, delays, loops e filas de tarefas assíncronas.
...
<Callout type=".">
`sleep(ms){:js}` é um **delay assíncrono**. Ideal para pausas temporizadas com `await`.
</Callout>
...
t.time(t.ms("3.5s"), () => {
  console.log("Executado após 3.5 segundos.");
});
```

<Callout type=".">
Executa uma **única vez** após o tempo definido (timeout).
</Callout>
...
<Callout type=".">
Executa o `callback` múltiplas vezes com intervalo fixo.
...
<Callout type=".">
Executa funções assíncronas **uma após a outra**, mesmo que tenham tempos diferentes.
</Callout>
...
| Método                | Tipo       | Exemplo                      | Descrição                                               |
| --------------------- | ---------- | ---------------------------- | ------------------------------------------------------- |
| `ms(value)`           | `function` | `t.ms("1m") → 60000`         | Converte string para milissegundos ou vice-versa.       |
| `sleep(ms)`           | `Promise`  | `await t.sleep(1000)`        | Espera o tempo especificado em milissegundos.           |
| `time(ms, cb)`        | `void`     | `t.time(3000, () => {})`     | Executa um callback após um tempo (timeout).            |
| `time(ms, times, cb)` | `void`     | `t.time(1000, 3, (i) => {})` | Executa o callback em loop, `n` vezes a cada intervalo. |
| `queue(tasks[])`      | `Promise`  | `await t.queue([...])`       | Executa funções assíncronas em ordem (em fila).         |
