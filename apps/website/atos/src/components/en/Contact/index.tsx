...
export default function Contact() {
  ...
  const [subject, setSubject] = useState('Report bugs');
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setStatus(res.ok ? 'Email sent successfully!' : `Error: ${json.error}`);
    } catch {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <label className="block">
            <span className="flex items-center gap-1"><FiUser /> Name</span>
            <input
              type="text"
              value={name}
          ...
          <label className="block">
            <span className="flex items-center gap-1"><FiMail /> Subject</span>
            <select
              value={subject}
          ...
          <label className="block">
            <span className="flex items-center gap-1"><FiMail /> Message</span>
            <textarea
              value={message}
          ...
          {status && (
            <p className="mt-4 text-center text-sm text-gray-700 dark:text-gray-300">{status}</p>
          )}
        </div>

        {/* Right column: Dropzone for Desktop */}
        <div
          className="hidden md:flex flex-col items-center justify-center border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 h-full cursor-pointer"
          onDragOver={e => e.preventDefault()}
          onDrop={handleDrop}
          onClick={() => fileInputRef.current?.click()}
        >
